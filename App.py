...
# Scopes (what data we can access)
...
def get_client_credentials_token():
    """Get token using Client Credentials (for public playlists)"""
    try:
...
def analyze():
    ...
    # If no user token, try client credentials token
    if not token:
    ...
    try:
        ...
        total_duration_ms = sum(item['track']['duration_ms'] for item in all_tracks_data if item.get('track'))
        total_duration_hours = round(total_duration_ms / (1000 * 60 * 60), 2)
        popularities = [item['track']['popularity'] for item in all_tracks_data if item.get('track')]
        avg_popularity = round(sum(popularities) / len(popularities)) if popularities else 0

        # Most popular
        most_popular = max(all_tracks_data, key=lambda x: x['track']['popularity'] if x.get('track') else 0)
        most_popular_track = most_popular['track']['name'] if most_popular.get('track') else 'N/A'
        most_popular_artist = most_popular['track']['artists'][0]['name'] if most_popular.get('track') and most_popular['track'].get('artists') else 'N/A'
...
def user_dashboard():
    ...
    headers = {'Authorization': f"Bearer {token}"}
    user_stats = {'top_artists': [], 'top_tracks': [], 'top_genres': []}
    ...
    try:
        ...
        # Get top tracks with album images
        ...
        if response_tracks.status_code == 200:
            ...
            for t in tracks:
                ...
                album_image = t['album']['images'][0]['url'] if t.get('album') and t['album'].get('images') else None
                user_stats['top_tracks'].append({
                    'name': f"{t.get('name', 'Unknown')} - {artist_name}",
                    'image': album_image
                })
            if tracks:
                top_track = tracks[0]
                personalized_insights.append(
                    f"Your current favorite track is '{top_track.get('name', 'Unknown')}' "
                    f"by {top_track['artists'][0]['name'] if top_track.get('artists') else 'Unknown'}."
                )

    except Exception:
        pass

    return jsonify({'user_stats': user_stats, 'personalized_insights': personalized_insights})
